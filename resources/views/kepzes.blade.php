            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                <table class="table-auto w-full border-collapse border border-gray-300">
                    ...
                    <tr class="bg-gray-200">
                        ...
                        <th class="border border-gray-300 px-4 py-2">Felvehető</th>
                        <th class="border border-gray-300 px-4 py-2">Minimum</th>
                        @if ($user->isAdmin())
                    ...
                    @foreach ($kepzesek as $kepzes)
                        <tr>
                            <td class="border border-gray-300 px-4 py-2">{{ $kepzes['nev'] }}</td>
                            <td class="border border-gray-300 px-4 py-2">{{ $kepzes['felveheto'] }}</td>
                            <td class="border border-gray-300 px-4 py-2">{{ $kepzes['minimum'] }}</td>
                            @if ($user->isAdmin())
                ...
                @if ($user->isAdmin())
                    <form action="/createKepzes" method="POST" class="mt-8 bg-white p-6 rounded-lg shadow-md border border-gray-200">
                        @csrf
                        <h3 class="text-lg font-semibold text-gray-800 mb-4">Új képzés létrehozása</h3>
